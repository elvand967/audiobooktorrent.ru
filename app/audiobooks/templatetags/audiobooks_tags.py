...
def page_title(
    ...
    if search_q:
        if search_results_count is not None:
            data['header_comment'] = 'По запросу: '
            data['title'] = search_q
            if search_results_count == 0:
                data['header_comment_end'] = 'нет записей'
                return data
            else:
                data['header_comment_end'] = 'найдено записей: '
                data['search_results_count'] = search_results_count
                return data
        else:
            data['header_comment'] = 'Результаты поиска по запросу: '
            data['title'] = search_q
            return data

    elif subcat_sel:
        subcategory = get_object_or_404(ModelSubcategories, slug=subcat_sel)
        data['header_comment'] = 'Жанр: '
        data['title'] = subcategory.name
        return data

    elif cat_sel:
        if cat_sel == "vse_zhanry":
            data['header_comment'] = 'Раздел: '
            data['title'] = 'Все жанры'
            return data
    ...
    data['title'] = 'Все жанры'
    return data  # или любой другой дефолтный текст
...
def breadcrumbs(
    ...
    if search_q:
        ...
        subcategory = get_object_or_404(ModelSubcategories, slug=subcat_sel)
        return f'Раздел: "{subcategory.name}"'
